...
class PhotoConnection2 : AppCompatActivity() {
    ...
    // only useful since we are doing a point to point connection
    private var currentEndpoint = "A"
    ...
    private val serviceConnection = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName?, service: IBinder?) {
            ...
            // finally, in the onCreate, suspend connection
            stepCounter_Pillar.stopDiscovery()
            stepCounter_Pillar.stopAdvertising()
            if(isBound) {
                Log.d("Thing", "MOD ON CALLED")
                modeOn()
            }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.d("ONCREATE-DESTROY", "photo oncreate called")
        Log.d("ONCREATE-DESTROY", "HAAAAAAAAAAAAAAAAAAAAAAAAA")
    ...
    private fun modeOff()
    {
        ...
        if(currentEndpoint != "A")
        {
            Nearby.getConnectionsClient(context).disconnectFromEndpoint(currentEndpoint)
        }
        currentEndpoint = "A"
...
//            val endpointId = resultData.getStringExtra(ENDPOINT_ID_EXTRA)
            ...
            /*
            // changes made here on 11/11 so we can use the finally block to call close()
            // on the pfd
            ...
            filePayload = try {
                ...
                // Close the ParcelFileDescriptor in the finally block to ensure it gets closed
                pfd?.close()
            }
            ...
            // 11/11 changes end.
            // the uri being set as non-nullable was part of this
             */
            val filePayload: Payload
            filePayload = try {
                // Open the ParcelFileDescriptor for this URI with read access.
        ...
        else
        {
            ...
            {
                //Log.d("CONINFO", "didn't select an image from activity")
            }
        }
    }
    ...
    private val endpointDiscoveryCallback: EndpointDiscoveryCallback =
        object : EndpointDiscoveryCallback() {
            override fun onEndpointFound(endpointId: String, info: DiscoveredEndpointInfo) {
                ...
                Log.d("Connection Report", "Endpoint $discoveredEndpointName found with id $discoveredEndpointID")
            ...
            override fun onEndpointLost(endpointId: String) {
                ...
                if (VERSION.SDK_INT >= VERSION_CODES.N) {
                    ...
                    Log.d("Connection Report", "Endpoint lost")
                }
            ...
            override fun onConnectionResult(endpointId: String, result: ConnectionResolution) {
                ...
                when (result.status.statusCode) {
                    ConnectionsStatusCodes.STATUS_OK -> {
                        ...
                        //connectionReport.text = "Connected to $endpointId"

                        Log.d("CONINFO", "Connected to $endpointId")
                    }
    ...
    private val payloadCallback: PayloadCallback = object : PayloadCallback() {
        ...
        private fun processFilePayload(payloadId: Long) {
            ...
            if (filePayload != null && filename != null) {
                ...
                // used to disconnect here, was removed.
            }
        }
    ...
    private fun makeConnection(endpointId: String)
    {
        ...
        if (links.isNotEmpty() && (endpointId != currentEndpoint)) {
            ...
            Nearby.getConnectionsClient(context)
                ...
                .addOnFailureListener(
                    OnFailureListener { e: java.lang.Exception? ->
                        Log.d("Make Connection", "Failed to connect ${e?.message}")

                    })
        }
        else if (endpointId == currentEndpoint)
        {
            Log.d(TAG, "Same Endpoint! Proceeding to send image")
            Log.d(TAG, "Same Endpoint! Proceeding to send image")
            showImageChooser(endpointId)
        }
        else
